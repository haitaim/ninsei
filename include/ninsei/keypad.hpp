...
public:
    ...
    [[nodiscard("Unused key check")]] inline bool is_down(Key_mask first, Key_masks... rest) const noexcept {
    ...
    template <typename... Key_masks>
    [[nodiscard("Unused key check")]] inline bool was_down(Key_mask first, Key_masks... rest) const noexcept {
        static_assert(std::conjunction_v<std::is_same<Key_mask, Key_masks>...>, "Only key masks accepted");
        std::uint32_t key_mask = make_mask(first, rest...);
    ...
    template <typename... Key_masks>
    [[nodiscard("Unused key check")]] inline bool is_hit(Key_mask first, Key_masks... rest) const noexcept {
        static_assert(std::conjunction_v<std::is_same<Key_mask, Key_masks>...>, "Only key masks accepted");
        std::uint32_t key_mask = make_mask(first, rest...);
    ...
    template <typename... Key_masks>
    [[nodiscard("Unused key check")]] inline bool is_held(Key_mask first, Key_masks... rest) const noexcept {
        static_assert(std::conjunction_v<std::is_same<Key_mask, Key_masks>...>, "Only key masks accepted");
        std::uint32_t key_mask = make_mask(first, rest...);
        return (current_key & previous_key & key_mask) == key_mask;
    }
    ...
    template <typename... Key_masks>
    [[nodiscard("Unused key check")]] inline bool is_released(Key_mask first, Key_masks... rest) const noexcept {
        static_assert(std::conjunction_v<std::is_same<Key_mask, Key_masks>...>, "Only key masks accepted");
        std::uint32_t key_mask = make_mask(first, rest...);
        return (~current_key & previous_key & key_mask) == key_mask;
    }
