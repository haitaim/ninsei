...
public:
    ...
    [[nodiscard("Unused key check")]] inline bool is_down(Key_mask first, Key_masks... rest) {
    ...
    template <typename... Key_masks>
    [[nodiscard("Unused key check")]] inline bool was_down(Key_mask first, Key_masks... rest) {
        static_assert(std::conjunction_v<std::is_same<Key_mask, Key_masks>...>, "Only key masks accepted");
        std::uint32_t key_mask = make_mask(first, rest...);
...
private:
    ...
    static constexpr std::uint32_t make_mask(Key_mask mask) {
        return static_cast<std::uint32_t>(mask);
    }

    template <typename... Key_masks>
    static constexpr std::uint32_t make_mask(Key_mask first, Key_masks ...rest) {
